...
async function triggerGitHubAction(token: string, owner: string, repo: string, workflow_id: string) {
  ...
  try {
    const response = await fetchWithTimeout(
      ...
      25000 // 25 second timeout for GitHub Action
    );
    ...
    if (error?.name === 'AbortError') {
      ...
      throw new Error('GitHub Action request timed out after 25 seconds');
    }
    throw error;
  }
}

export const maxDuration = 60; // Set maximum duration to 1 minute (Hobby plan limit)
  ...
  try {
    ...
    try {
      const customUrlResponse = await fetchWithTimeout(
        ...
        25000 // 25 second timeout for custom API
      );
      ...
      if (error?.name === 'AbortError') {
        ...
        throw new Error('Custom API request timed out after 25 seconds');
      }
      throw error;
    }
