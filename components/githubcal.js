...
async function fetchChartData(calendarName) {
    try {
        // Updated to use calendar-events.json instead of individual files
        ...
        // Filter data based on calendar type
        const filteredData = data.filter(event => event.calendar === calendarName)
            .map(event => ({
                date: event.date.split('T')[0], // Extract just the date part
                value: 1 // Each event counts as 1
            }));
            
        console.log(`Fetched data for ${calendarName}:`, filteredData);
        return filteredData;
    } catch (error) {
...
function processRoutinesData(data, eventType) {
    return data.filter(item => {
        const eventName = item.summary?.toLowerCase() || '';
        return eventType === 'read' ? 
            eventName === "read" : 
            eventName === "daily 60*60*3";
    }).map(item => ({
...
async function initializeCalendars() {
    try {
        // Updated calendar names to match your JSON structure
